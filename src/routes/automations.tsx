...
function ClayFindCompaniesReplica() {
  ...
  const handleSearch = async () => {
    setIsSearching(true);
    try {
      const response = await fetch((import.meta.env.VITE_BACKEND_URL || "https://whale-app-muqf3.ondigitalocean.app") + "/api/automation/search", {
        method: "POST",
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      const response = await fetch((import.meta.env.VITE_BACKEND_URL || "https://whale-app-muqf3.ondigitalocean.app") + "/api/automation/enrich", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
