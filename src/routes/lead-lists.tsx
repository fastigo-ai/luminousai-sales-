...
function LeadListsPage() {
  ...
  const fetchSpreadsheetData = async () => {
    try {
      const res = await fetch((import.meta.env.VITE_BACKEND_URL || "https://whale-app-muqf3.ondigitalocean.app") + "/api/automation/spreadsheet");
      if (res.ok) {
  ...
  const handleApolloSearch = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch((import.meta.env.VITE_BACKEND_URL || "https://whale-app-muqf3.ondigitalocean.app") + "/api/automation/search", {
        method: "POST",
  ...
  const selectApolloCompany = async (company: any) => {
    ...
    if (website) {
      ...
      try {
        const res = await fetch((import.meta.env.VITE_BACKEND_URL || "https://whale-app-muqf3.ondigitalocean.app") + "/api/automation/search_people", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDirectPitch = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch((import.meta.env.VITE_BACKEND_URL || "https://whale-app-muqf3.ondigitalocean.app") + "/api/automation/enrich_and_pitch", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleEnrichAll = async () => {
    ...
    try {
      const res = await fetch((import.meta.env.VITE_BACKEND_URL || "https://whale-app-muqf3.ondigitalocean.app") + "/api/automation/enrich_leads", {
        method: "POST"
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const handleCopilotSuggest = async (leadId: number) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL || "https://whale-app-muqf3.ondigitalocean.app"}/api/copilot/suggest/${leadId}`);
      if (res.ok) {
        const data = await res.json();
        if (data.error) {
  ...
  const handleDeleteLead = async (leadId: number) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL || "https://whale-app-muqf3.ondigitalocean.app"}/api/automation/leads/${leadId}`, {
        method: "DELETE"
      });
      if (res.ok) {
